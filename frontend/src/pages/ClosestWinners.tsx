...
function ClosestWinners() {
    ...
    return (
        <div className="close-winners-container overview-section">
            ...
            <p>Wählen Sie eine Partei aus, um die zehn knappsten Sieger für diese Partei anzuzeigen. Wenn diese Partei keine 10 Direktmandate erzielen konnte, werden entsprechend viele knappste Verlierer angezeigt.</p>
            <select value={selectedParty} onChange={handleChange}>
                ...
                <option key={partei} value={partei}>{partei}</option>
                ))}
            </select>

            {
                selectedParty && Object.entries(groupedData).map(
                    ([partei, data]) => {
                        ...
                        return(
                            <div key={partei} className="table-container">
                                ...
                                {
                                    data.filter(elem => elem.tag === 'knappsterVerliererVsSieger').length > 0 && (
                                        <div key={partei} className="table-container">
