...
BEGIN
    ...
    WITH gesamtstimmen as (
      SELECT SUM(anzahlStimmen) as gesamtstimmen, SUM(anzahlStimmen)*1.0/100*5 as fuenfProzentHuerde
      FROM gesamtStimmenProPartei
    ),

    parteienÜberFünfProzent(parteiid, gesamtstimmen, fuenfProzentHuerde) as (
      SELECT parteiid, gspp.anzahlStimmen, fuenfProzentHuerde
      FROM gesamtStimmenProPartei gspp, gesamtstimmen
      WHERE gspp.anzahlStimmen > fuenfProzentHuerde
    ),

    gesamtstimmenDerÜberFünfProzentParteienProWahlkreis as (
      SELECT g.parteiid, g.wahlkreisid, g.anzahlStimmen
    ...
    stimmenProParteiProWahlkreisMitAnzSitze as (
      SELECT g.parteiid, g.wahlkreisid, g.anzahlStimmen as gesamtstimmen, w.anzahlSitze
