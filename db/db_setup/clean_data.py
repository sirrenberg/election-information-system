...
def clean_data_2018():
    ...
    for i in range (1, 8):
        file_path = f'../data/clean_data_2018/90{i}.xlsx'
        empty_df.to_excel(file_path, index=False)


    iterate_over_sheets("../data/data_2018/BewerberNachStimmkreisen_901_3010_120946813.xls",1, parties_2018, 2018)
    iterate_over_sheets("../data/data_2018/BewerberNachStimmkreisen_902_3010_120949031.xls",2, parties_2018, 2018)
    iterate_over_sheets("../data/data_2018/BewerberNachStimmkreisen_903_3010_120951000.xls",3, parties_2018, 2018)
    
    iterate_over_sheets("../data/data_2018/BewerberNachStimmkreisen_904_3010_120953304.xls",4, parties_2018, 2018)
    iterate_over_sheets("../data/data_2018/BewerberNachStimmkreisen_905_3010_120956931.xls",5, parties_2018, 2018)
    iterate_over_sheets("../data/data_2018/BewerberNachStimmkreisen_906_3010_120959690.xls",6, parties_2018, 2018)

    iterate_over_sheets("../data/data_2018/BewerberNachStimmkreisen_907_3010_121003563.xls",7, parties_2018, 2018)
...
def clean_data_2023():
    ...
    #Save the empty DataFrame to an Excel file
    for i in range (1, 8):
        file_path = f'../data/clean_data_2023/90{i}.xlsx'
        empty_df.to_excel(file_path, index=False)

    for i in range(1, 8):
        iterate_over_sheets(f"../data/data_2023/LTW2023_BEWERBER_UND_ABGEORDNETE_WkrNr_90{i}.xls", i, parties_2023, 2023)
...
def iterate_over_sheets(file_path,wk_index, parties, year):
    ...
    for i in range(1,sheet_number+1):
        ...
        stimmkreise_nummern = find_stimmkreis_nummer(df_sheet)

        num_cols= len(df_sheet.columns)
        
        #Determine row indices, where candidates of a new party start
        indices = find_row_index_party(df_sheet, parties)
        ...
        for i in range(0, len(indices)):
            if(i==len(indices)-1):
                ...
                if(df_subframe.empty):
                    print(f"Why are you empty my frind, index {indices[i]}, sheet {sheet_name}")
                    break

                cleaned_df = modify_df(df_subframe, start_index2, stimmkreise_nummern, num_cols)

                with pd.ExcelWriter(f'../data/clean_data_{year}/90{wk_index}.xlsx', mode='a', engine='openpyxl') as writer:
...
def modify_df(df, start_index, stimmkreise_nummern, num_cols):
    ...
    result_df = rename_cols(result_df, stimmkreise_nummern, num_cols)
    result_df = add_party_to_candidates(result_df)
