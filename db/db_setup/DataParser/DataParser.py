...
class DataParser: 
    ...
    def create_insert_queries(year):
        ...
        if not os.path.exists(kandidaten_file):
            #Add dummy kandidat for each party, collecting zweitstimmen without direct candidate
            dummy_kandidaten = ""
            for key, value in DataParser.parties.items():
                kandidat_id = -value
                kandidat_party = value
                
                dummy_kandidaten += f"({kandidat_id}, 'Sonstige', {kandidat_party}),\n"

            print(dummy_kandidaten)
            with open(kandidaten_file, "wb") as file:
                sql = f"INSERT INTO kandidaten VALUES \n"
                
                file.write(sql.encode('utf-8', 'ignore'))
                file.write(dummy_kandidaten.encode('utf-8', 'ignore'))
    ...
    def read_sheets_and_create_query(excel_file, date, year, wk_nummer):
        ...
        for i in range(0, len(data.sheet_names)-1):
            ...
            for _, row in current_sheet.iterrows():
                #Row, containing zweitstimmen without direct candidates
                if row['Id'] == "besonderen Bewerbers":
                        query_to_append = DataParser.create_insert_query_zweitstimmen_without_candidate(row, current_sheet.columns, date)
                ...
                try:
                    ...
                    if(type(value) == float and not math.isnan(value)):
                        ...
                        query_to_append = DataParser.create_insert_query(row, current_sheet.columns, date, wk_nummer)
    ...
    @staticmethod
    def create_insert_query(df_row, df_cols, date, wk_nummer):
        """
    ...
    @staticmethod
    def create_insert_query_zweitstimmen_without_candidate(df_row, df_cols, date):
        """
        Create SQL insert query for zweitstimmen without direct candidate
        """
        kandidat_party = df_row['Partei']
        ...
        #Set id to default value of -party_id
        kandidat_id = - party_id
        ...
        for i in range(4, len(df_row)):
            ...
            #Only zweitstimmen could be without direct candidate
            insert_values_zweitstimmen += f"({kandidat_id}, {sk_nummer}, '{date}', {int(zweitstimmen_str)}),\n"    
           
        return ["", insert_values_zweitstimmen]
